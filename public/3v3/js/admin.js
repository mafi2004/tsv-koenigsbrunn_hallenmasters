/* admin.js – Administrationslogik für den 3v3‑Modus
-------------------------------------------------------------------------------
Diese Datei bündelt die komplette Admin‑Logik für das 3v3‑Turniersystem.

Hauptaufgaben:
- Logo laden + Fehleranzeige
- QR‑Code‑Generator + Viewer‑Link
- API‑Wrapper für alle Backend‑Routen
- State‑Management (Teams, Matches, History, Snapshots, Ops)
- Zeitplan‑Management (localStorage)
- Hilfsfunktionen für UI‑Feedback

Die Datei wird von admin.html eingebunden und steuert sämtliche Interaktionen.
------------------------------------------------------------------------------- */


/* ============================================================================
   LOGO-LADEN
   - Lädt das Vereinslogo aus /assets
   - Zeigt Fehlerhinweis, falls Datei fehlt
   - Nutzt Cache‑Bust, damit Browser immer die aktuelle Version lädt
============================================================================ */
...
/* ============================================================================
   QR‑CODE + VIEWER‑LINK
   - Speichert Viewer‑Basis (IP/Host) im localStorage
   - Baut Viewer‑URL dynamisch (lokal oder Render‑Deployment)
   - Erzeugt QR‑Code über /api/qr
============================================================================ */
...
/**
 * Baut die Viewer‑URL:
 * - Wenn Benutzer eine IP/Domain eingibt → http(s)://host/3v3/viewer.html
 * - Sonst Fallback auf Render‑Deployment
 */
function buildViewerUrl(base) {
  const host = (base || '').trim();

  // Wenn Benutzer http:// oder https:// angibt → übernehmen
  ...
  // Fallback: automatische Erkennung des Modus
...
/* ============================================================================
   API‑WRAPPER
   - Einheitliche safeFetch‑Funktion
   - Alle API‑Routen als Funktionen gekapselt
   - Erleichtert Fehlerbehandlung und Wiederverwendung
============================================================================ */
 ...
 * - Führt Fetch aus
 * - Prüft HTTP‑Status
 * - Gibt JSON zurück oder wirft Fehler
 */
...
const apiTeamAdd        = (name, groupName) =>
  safeFetch('/teams', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ name, groupName })
  });
const apiTeamDelete     = (id) =>
...
/* --- Ergebnisse --- */
const apiWinnerSet = (matchId, winnerTeamId) =>
  safeFetch('/results/winner', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ matchId, winner: winnerTeamId })
  });
...
/* --- Historie --- */
const apiHistoryLatest = (group) =>
  safeFetch('/history/latest/' + encodeURIComponent(group), { method:'GET' });

const apiHistorySetWinner = (historyId, winner) =>
  safeFetch('/history/' + encodeURIComponent(historyId) + '/winner', {
    method:'PATCH',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ winner })
  });

const apiRebuildCurrRound = (group) =>
  safeFetch('/funino/rebuildCurrentRound', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ groupName: group })
  });

/* --- Admin‑Operationen & Snapshots --- */
const apiOpsList       = (limit=200) => safeFetch('/adminOps/ops?limit=' + encodeURIComponent(limit));
const apiSnapList      = () => safeFetch('/adminOps/snapshots');
...
const apiMetaSet = (yearLabel, schedule) =>
  safeFetch('/meta', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ yearLabel, schedule })
  });

/* Aliases für bequemere Verwendung */
...
function saveSchedule(cfg) {
  localStorage.setItem(SCHED_KEY, JSON.stringify(cfg));
}

/**
 * Gibt gültigen Zeitplan zurück oder null.
 * Prüft:
 * - HH:MM Format
 * - Dauer > 0
 * - Pause >= 0
...
/* ============================================================================
   ZEITPLAN-UI (UI ↔ localStorage ↔ Backend)
   - Füllt UI-Felder aus gespeichertem Zeitplan
   - Validiert Eingaben
   - Speichert Zeitplan lokal
   - Sendet Zeitplan an Backend (/meta + /schedule/recalculate)
============================================================================ */

/** Überträgt gespeicherten Zeitplan in die UI-Felder */
...
/** Event-Handler für "Speichern" im Zeitplanbereich */
function wireScheduleUI() {
  ...
  btn && btn.addEventListener('click', async () => {
    ...
    if (!/^\d{2}:\d{2}$/.test(timeHHMM)) {
      showMsg('#timeMsg', 'Bitte Startzeit im Format HH:MM setzen.', true);
      return;
    }
    if (!Number.isFinite(dur) || dur <= 0) {
      showMsg('#timeMsg', 'Dauer (min) ungültig.', true);
      return;
    }
    if (!Number.isFinite(brk) || brk < 0) {
      showMsg('#timeMsg', 'Pause (min) ungültig.', true);
      return;
    }
...
/**
 * Erzeugt das Team-Grid dynamisch für alle vorhandenen Gruppen.
 * Jede Gruppe erhält eine eigene Karte + Tabelle.
 */
function buildTeamsGrid(groups) {
  ...
  groupsOrdered.forEach(g => {
    ...
    badge.className = 'pill ' + groupClass(g);
  ...
  document.querySelectorAll('[id^="teams-table-"]').forEach(tbl => {
    tbl.addEventListener('click', async (e) => {
      ...
      /* --- TEAM EDITIEREN (in Input-Feld umwandeln) --- */
...
/**
 * Rendert alle Teams in die zuvor erzeugten Tabellen.
 */
...
/**
 * Lädt Teams neu vom Backend und rendert sie.
 * Sendet optional ein Socket‑Event an Viewer/Admin‑Clients.
 */
...
/* ============================================================================
   MATCHES – SPIELTABELLE (Sieger-Spalte entfernt)
   - Rendert alle Spiele in der Tabelle
   - Zeigt Gewinner mit 🏆‑Icon
   - Buttons zum Setzen des Siegers
   - Lädt Matches neu nach Änderungen
============================================================================ */
function renderMatches() {
  ...
  if (!Array.isArray(MATCHES)) {
    showMsg('#nextMsg', 'Spieldaten ungültig (kein Array).', true);
    return;
  }
  ...
  MATCHES.forEach(m => {
    ...
    const r  = m?.round ?? '';
    const f  = m?.field ?? '';
    const taId = m?.teamA_id ?? null;
    const tbId = m?.teamB_id ?? null;

    const taName = m?.teamA ?? (taId != null ? String(taId) : '');
    const tbName = m?.teamB ?? (tbId != null ? String(tbId) : '');

    const planned = m?.plannedStart ?? '–';
    const w = m?.winner;
    ...
    /* --- Team A mit Sieger-Icon --- */
    const tdA = document.createElement('td');
    const wrapA = document.createElement('span');
    wrapA.className = 'teamNameWrap';

    const winA = hasW && Number(w) === Number(taId);
    if (winA) {
      const ico = document.createElement('span');
      ico.className = 'winIcon';
      ico.textContent = '🏆';
      ico.setAttribute('aria-label', 'Sieger');

      const txt = document.createElement('span');
      txt.className = 'winnerText';
      txt.textContent = taName;

      wrapA.append(ico, txt);
    } else {
      const txt = document.createElement('span');
      txt.className = 'loserText';
      txt.textContent = taName;
      wrapA.append(txt);
    }
    tdA.replaceChildren(wrapA);

    /* --- Team B mit Sieger-Icon --- */
    const tdB = document.createElement('td');
    const wrapB = document.createElement('span');
    wrapB.className = 'teamNameWrap';

    const winB = hasW && Number(w) === Number(tbId);
    if (winB) {
      const ico = document.createElement('span');
      ico.className = 'winIcon';
      ico.textContent = '🏆';
      ...
      const txt = document.createElement('span');
      txt.className = 'winnerText';
      txt.textContent = tbName;

      wrapB.append(ico, txt);
    } else {
      const txt = document.createElement('span');
      txt.className = 'loserText';
      txt.textContent = tbName;
    ...
    /* --- Aktion: Sieger setzen --- */
    const tdAct = document.createElement('td');
    tdAct.className = 'row';

    const btnA = document.createElement('button');
    btnA.className = 'btn ' + groupClass(g);
    btnA.type = 'button';
    btnA.dataset.action = 'winA';
    btnA.dataset.id = String(id);
    if (taId != null) btnA.dataset.team = String(taId);
    btnA.textContent = 'Sieger: Team A';

    const btnB = document.createElement('button');
    btnB.className = 'btn ' + groupClass(g);
    btnB.type = 'button';
    btnB.dataset.action = 'winB';
    btnB.dataset.id = String(id);
    if (tbId != null) btnB.dataset.team = String(tbId);
    btnB.textContent = 'Sieger: Team B';
  ...
  tbody.querySelectorAll('button[data-action]').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (!Number.isFinite(id) || !Number.isFinite(winnerId)) {
        alert('Ungültige Match-/Team-ID.');
        return;
      }

      try {
        await setWinner(id, winnerId);
        await refreshMatches();
      } catch (err) {
        alert('Fehler beim Setzen des Siegers: ' + err.message);
      }
    });
  });
}

/** Lädt Matches neu und rendert sie */
async function refreshMatches() {
  try {
    MATCHES = await loadMatches();
    renderMatches();
  } catch (e) {
    showMsg('#nextMsg', 'Fehler: ' + e.message, true);
  }
}


/* ============================================================================
   HISTORY – LETZTE GESPIELTE RUNDE
...
function renderHistory() {
  ...
  if (!items.length) {
    ...
    td.colSpan = 6;
    td.textContent = 'Keine History vorhanden.';
    tr.appendChild(td);
    tbody.appendChild(tr);
    return;
  }
  ...
  items.forEach((h, idx) => {
    ...
    if (aIsW) {
      ...
      const txt = document.createElement('span');
      txt.className = 'winnerText';
      txt.textContent = h.teamA_name ?? h.teamA ?? '';
    ...
    if (bIsW) {
      ...
      const txt = document.createElement('span');
      txt.className = 'winnerText';
      txt.textContent = h.teamB_name ?? h.teamB ?? '';
    ...
    const btnA = document.createElement('button');
    btnA.className = 'btn ' + groupClass(h.groupName);
    btnA.type = 'button';
    btnA.textContent = 'Sieger: Team A';
    btnA.addEventListener('click', async () => {
      try {
        await setHistoryWinner(h.id, h.teamA);
        showMsg('#histMsg', `#${idx + 1}: Sieger → Team A gespeichert.`);
        await loadHistoryUI();
      } catch (e) {
        showMsg('#histMsg', 'Fehler: ' + e.message, true);
      }
    });

    const btnB = document.createElement('button');
    btnB.className = 'btn ' + groupClass(h.groupName);
    btnB.type = 'button';
    btnB.textContent = 'Sieger: Team B';
    btnB.addEventListener('click', async () => {
      try {
        await setHistoryWinner(h.id, h.teamB);
        showMsg('#histMsg', `#${idx + 1}: Sieger → Team B gespeichert.`);
        await loadHistoryUI();
      } catch (e) {
        showMsg('#histMsg', 'Fehler: ' + e.message, true);
      }
...
/** Lädt History für ausgewählte Gruppe */
...
/** Setzt History zurück (UI-seitig) */
function clearHistoryUI() {
  HISTORY = {
    groupName: (document.getElementById('hist-group')?.value || '').toUpperCase(),
    round: null,
    batchId: null,
    items: []
  };
  ...
  if (el) {
    el.style.display = 'inline-block';
    el.textContent = 'History zurückgesetzt.';
...
/* ============================================================================
   SNAPSHOTS & ADMIN-OPS
   - Snapshotliste rendern
   - Admin‑Operationen rendern
   - Laden/Aktualisieren
============================================================================ */
function renderSnapshots() {
  ...
  if (!Array.isArray(SNAPSHOTS) || SNAPSHOTS.length === 0) {
    ...
    opt.value = '';
    opt.textContent = '– keine Snapshots –';
    sel.appendChild(opt);
    return;
  }
  ...
  hint.value = '';
  hint.textContent = 'Snapshot wählen …';
  hint.disabled = true;
  hint.selected = true;
  ...
  SNAPSHOTS.forEach(s => {
    ...
    opt.textContent = `#${s.id} · ${fmtTs(s.ts)} · ${s.path}`;
    sel.appendChild(opt);
  });
}

function renderOps() {
  const sel = document.getElementById('opsSelect');
  const detailsBox = document.getElementById('opDetails');
  if (!sel || !detailsBox) return;
  ...
  if (!Array.isArray(OPS) || OPS.length === 0) {
    const opt = document.createElement('option');
    opt.value = '';
    opt.textContent = '– keine Operationen –';
    sel.appendChild(opt);
    return;
  }
  ...
  const hint = document.createElement('option');
  hint.value = '';
  hint.textContent = 'Operation wählen …';
  hint.disabled = true;
  hint.selected = true;
  sel.appendChild(hint);
  ...
  OPS.slice().reverse().forEach(op => {
    ...
    opt.textContent = `#${op.id} · ${op.op} · ${fmtTs(op.ts)}`;
    opt.dataset.payload = op.payload ?? '';
    opt.dataset.status  = op.status ?? '';
    sel.appendChild(opt);
  });
}
...
async function refreshSnapshots() {
  try {
    ...
    showMsg('#recMsg', 'Snapshots laden fehlgeschlagen: ' + e.message, true);
  }
}

async function refreshOps() {
  try {
    OPS = await loadOps(200);
    renderOps();
  } catch (e) {
    showMsg('#recMsg', 'Ops laden fehlgeschlagen: ' + e.message, true);
  }
}


/* ============================================================================
   NEXT ROUND – PAYLOAD ERMITTELN
   - Liest die obersten 3 Spiele
   - Prüft, ob alle zur gleichen Gruppe gehören
   - Prüft, ob Sieger gesetzt sind
   - Liefert { groupName, results[] }
============================================================================ */
function computeNextRoundPayloadFromTop3() {
  if (!Array.isArray(MATCHES)) {
    throw new Error('Keine Spieldaten geladen.');
  }

  const top3 = MATCHES.slice(0, 3);
  if (top3.length !== 3) {
    throw new Error('Mindestens 3 Spiele erforderlich.');
  }

  const groupName = String(top3[0]?.groupName || '').trim();
  if (!groupName) {
    throw new Error('Gruppe der obersten Spiele nicht ermittelbar.');
  }

  const allSame = top3.every(m =>
    String(m?.groupName || '').trim() === groupName
  );
  if (!allSame) {
    throw new Error('Die obersten 3 Spiele gehören nicht zur gleichen Gruppe.');
  }
  ...
  const results = top3.map(m => {
    ...
    if (wRaw == null) {
      throw new Error('Für Spiel ' + (m?.id || '?') + ' ist kein Sieger gesetzt.');
    }

    const winnerId = Number(wRaw);
    if (!Number.isFinite(winnerId)) {
      throw new Error('Ungültiger Siegerwert bei Spiel ' + (m?.id || '?'));
    }

    const loserId =
      (Number.isFinite(teamAId) && winnerId === teamAId) ? teamBId :
      (Number.isFinite(teamBId) && winnerId === teamBId) ? teamAId :
      NaN;

    if (!Number.isFinite(loserId)) {
      throw new Error('Sieger gehört nicht zu Team A/B bei Spiel ' + (m?.id || '?'));
    }

    return {
      winnerId,
      loserId,
      field: Number(m?.field) || undefined
    };
  });
...
/* ============================================================================
   SOCKET.IO – ADMIN-KANAL
   - Verbindet Admin-Client mit Namespace /minis3
   - Empfängt Live-Events vom Backend
   - Aktualisiert Matches, History, Snapshots, Meta-Daten
   - Reagiert auf Reset-Ereignisse
============================================================================ */
function initSocket() {
  const s = io("/minis3", {
    path: "/socket.io",
  ...
  /* --- Runde neu aufgebaut (Rebuild) --- */
  s.on('round:rebuilt', async () => {
    await reloadMatches();
    await loadHistoryUI();
  });

  /* --- Zeitplan neu berechnet --- */
  s.on('schedule:recalculated', reloadMatches);

  /* --- Matches zurückgesetzt --- */
  s.on('matches:reset', () => {
    reloadMatches();
    clearHistoryUI();
  });

  /* --- Modul-Reset / Komplett-Reset --- */
  s.on("reset-3v3", () => { location.reload(); });
  s.on("reset-all", () => { location.reload(); });

  /* --- Snapshot erstellt --- */
  s.on('snapshot:created', async () => {
    await refreshSnapshots();
    showMsg('#recMsg', 'Snapshot erstellt.');
  });
  ...
  /* --- Meta-Daten aktualisiert (Jahrgang + Zeitplan) --- */
  s.on('meta:updated', (p) => {
...
/* ============================================================================
   UI-VERDRAHTUNG
   - Bindet alle Buttons an ihre Funktionen
   - Teams, Matches, History, Snapshots, Import/Export, Zeitplan
============================================================================ */
function wireUI() {

  /* --- QR‑Code / Viewer‑Basis --- */
  wireQRBase();

  /* ==========================================================================
     TEAMS
  ========================================================================== */
  document.getElementById('btnLoadTeams')
    ?.addEventListener('click', refreshTeams);

  document.getElementById('btnDeleteAllTeams')
    ?.addEventListener('click', async () => {
      if (!confirm('Wirklich ALLE Teams löschen?')) return;
      try {
        await deleteAllTeams();
        showMsg('#teamsMsg', 'Alle Teams wurden gelöscht.');
        await refreshTeams();
      } catch (err) {
        showMsg('#teamsMsg', 'Fehler beim Löschen aller Teams: ' + err.message, true);
      }
    });
  ...
  ========================================================================== */
  document.getElementById('btnImportTournament')
    ?.addEventListener('click', async () => {
  ...
  document.getElementById('btnExportTournament')
    ?.addEventListener('click', exportTournamentFile);

  /* ==========================================================================
     MATCHES
  ========================================================================== */
  document.getElementById('btnLoadMatches')
    ?.addEventListener('click', refreshMatches);
  ...
  /* Gruppen starten */
  document.getElementById('btnStartA')
    ?.addEventListener('click', async () => {
      try { await startGroup('A'); await refreshMatches(); }
      catch (err) { alert('Fehler: ' + err.message); }
    });

  document.getElementById('btnStartB')
    ?.addEventListener('click', async () => {
      try { await startGroup('B'); await refreshMatches(); }
      catch (err) { alert('Fehler: ' + err.message); }
    });

  document.getElementById('btnStartC')
    ?.addEventListener('click', async () => {
      try { await startGroup('C'); await refreshMatches(); }
      catch (err) { alert('Fehler: ' + err.message); }
    });
  ...
  document.getElementById('btnNextRound')
    ?.addEventListener('click', async () => {
      try {
        ...
        showMsg('#nextMsg', 'Fehler: ' + err.message, true);
      }
    });
  ...
  /* ==========================================================================
     HISTORY
  ========================================================================== */
  document.getElementById('btnLoadHistory')
    ?.addEventListener('click', loadHistoryUI);

  document.getElementById('btnApplyHistory')
    ?.addEventListener('click', async () => {
      const g = (document.getElementById('hist-group').value || '').trim().toUpperCase();
      try {
  ...
  /* ==========================================================================
     SNAPSHOTS & RECOVERY
  ========================================================================== */
  document.getElementById('btnMakeSnapshot')
    ?.addEventListener('click', async () => {
      try {
        const r = await makeSnapshot();
        showMsg('#recMsg', `Snapshot erstellt: ${r.path}`);
        await refreshSnapshots();
      } catch (e) {
        showMsg('#recMsg', 'Snapshot-Fehler: ' + e.message, true);
      }
    });

  document.getElementById('btnRecoverLatest')
    ?.addEventListener('click', async () => {
      ...
      try {
        ...
        showMsg('#recMsg', 'Recovery-Fehler: ' + e.message, true);
      }
    });

  document.getElementById('btnRecoverSelected')
    ?.addEventListener('click', async () => {
      const sel = document.getElementById('snapshotSelect');
      const id = Number(sel?.value);

      if (!Number.isFinite(id)) {
        showMsg('#recMsg', 'Bitte einen Snapshot wählen.', true);
        return;
      }
...
/* ============================================================================
   ADMIN-OP DETAILS
   - Zeigt Details einer ausgewählten Admin‑Operation (Payload, Status)
   - Optional: Popup-Anzeige
============================================================================ */
document.getElementById('opsSelect')?.addEventListener('change', (e) => {
  ...
  try {
    ...
    payloadText = JSON.stringify(obj, null, 2);
  } catch {}
...
/** Popup-Anzeige der Operation */
document.getElementById('btnShowOp')?.addEventListener('click', () => {
  const sel = document.getElementById('opsSelect');
  if (!sel?.value) {
    showMsg('#recMsg', 'Bitte eine Operation wählen.', true);
    return;
  }
...
/* ============================================================================
   RESEED (Gruppen D/E/F)
   - Baut nach 3 Runden neue Gruppen
   - Zeigt Anzahl erzeugter Spiele pro Gruppe
============================================================================ */
...
btn?.addEventListener('click', async () => {
  ...
  try {
    ...
    showMsg(
      msgEl,
      (r?.msg || 'Gruppen neu zusammengestellt.') +
      ` (D:${d}, E:${e}, F:${f})`
    );
    ...
    showMsg(msgEl, 'Reseed-Fehler: ' + (e?.message || e), true);
  }
});


/* ============================================================================
   ZEITPLAN-UI INITIALISIEREN
============================================================================ */
wireScheduleUI();
fillScheduleUI();
...
/* ============================================================================
   TABS – Navigation zwischen Teams / Spiele / Recovery
============================================================================ */
function setupTabs() {
  const tabs = [
...
/* ============================================================================
   ...
   - Liest JSON-Datei
   - Validiert Struktur
   - Setzt Jahrgang, Zeitplan, Teams
   - Aktualisiert Backend (Meta + Schedule)
============================================================================ */
async function importTournamentFile(file) {
  if (!file) {
    showMsg('#tournamentMsg', 'Bitte Turnierdatei wählen.', true);
    return;
  }
  ...
  try {
    ...
    if (
      !s ||
      !/^\d{2}:\d{2}$/.test(s.timeHHMM) ||
      !Number.isFinite(Number(s.dur)) || Number(s.dur) <= 0 ||
      !Number.isFinite(Number(s.brk)) || Number(s.brk) < 0
    ) {
    ...
    for (const t of teamsArr) {
      ...
      try {
        await addTeam(name, grp);
        added++;
      } catch {}
    }

    /* --- Meta-Fallback (ältere Dateien) --- */
    ...
    showMsg(
      '#tournamentMsg',
      `Turnierdatei importiert. Label: ${yearLabel ?? '–'} | Teams: ${added}/${teamsArr.length}`
    );
...
/* ============================================================================
   TURNIERDATEI EXPORT
   - Exportiert Meta + Zeitplan + Teams als JSON
   - Dateiname enthält Jahrgang + Datum + Uhrzeit
============================================================================ */
async function exportTournamentFile() {
  try {
    ...
    let meta = null;
    try { meta = await apiMetaGet(); } catch {}
    ...
    const payload = {
      meta: {
        yearLabel: labelFromUI ?? YEAR_LABEL ?? meta?.yearLabel ?? null,
        schedule
      },
      teams: (teams || []).map(t => ({
        name: t.name,
    ...
    const fn = `tournament_${(payload.meta.yearLabel || 'label')}_${YYYY}${MM}${DD}_${hh}${mm}.json`
      .replace(/\s+/g, '_');
...
/* ============================================================================
   ...
   - Overlay blockiert Admin-Bereich
   - Passwort wird lokal gespeichert
============================================================================ */
...
/* ============================================================================
   ...
   - QR‑Code
   - Meta laden
   - Teams/Matches/History/Snapshots/OPS laden
============================================================================ */
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // QR‑Code initial setzen
  applyQRBaseToUI();

  // Meta laden (Jahrgang + Zeitplan)
  try {
    const m = await apiMetaGet();
    setAdminYearLabel(m?.yearLabel ?? null);

    const fld = document.getElementById('yearLabelInput');
    if (fld) fld.value = (m?.yearLabel ?? '');
  ...
  // Matches alle 15 Sekunden aktualisieren
