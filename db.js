...
db.serialize(() => {
  ...
  // Prüfen, ob matches-Tabelle existiert
  db.get(`SELECT name FROM sqlite_master WHERE type='table' AND name='matches'`, (err, row) => {
    ...
    // Wenn es noch keine matches-Tabelle gibt, direkt Ziel-Schema anlegen
    if (!row) {
    ...
    // Es gibt bereits matches -> Spalten prüfen und ggf. migrieren
