 ...
 * Effekt: setzt plannedStart für alle aktuell vorhandenen Gruppen (basierend auf group_state.lastRound) neu,
 *         gemäß globalem ABC-ABC… Raster.
 */
router.post('/recalculate', (req, res) => {
  ...
  // 1) Alphabetische Gruppenliste aus TEAMS (stabiler Index pro Gruppe)
  db.all(
    ...
    (eG, rowsG) => {
      ...
      // 2) vorhandene Gruppen mit Status bestimmen (nur die, die Matches haben)
      db.all(`SELECT DISTINCT UPPER(groupName) AS g FROM matches`, [], (eM, rowsM) => {
        ...
        // 3) Runde je aktiver Gruppe aus group_state lesen (Fallback 1)
        db.all(`SELECT groupName, lastRound FROM group_state`, [], (eS, rowsS) => {
          ...
          db.run(`BEGIN IMMEDIATE`, (eBegin) => {
            ...
            for (const g of activeGroups) {
              ...
              if (idx < 0) continue; // Gruppe nicht (mehr) in TEAMS -> überspringen
              ...
              // setze geplante Zeit für ALLE Spiele dieser aktiven Gruppe (aktueller Satz = 3 Zeilen)
              db.run(
